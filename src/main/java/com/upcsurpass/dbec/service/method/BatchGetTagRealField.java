...
public class BatchGetTagRealField extends SynchronizedMethod {
	private static final Logger LOGGER = LoggerFactory.getLogger(BatchGetTagRealField.class);

	/**
	 * 这里应该修改一下，自动处理，而不应该留给用户
	 * @param pnsc
	 */
	protected BatchGetTagRealField(PooledNIOSocketClient pnsc) {
		super(pnsc);
	}

	public BatchGetTagRealField(PooledNIOSocketClient pnsc,String fieldName,List<Integer> Ids){
		super(pnsc);
	...
	public SocketExchange setSocketExchange() {
		...
		SocketExchange se = new SocketExchange(GlobalConsts.DBEC_BATCHGETTAGREALFIELD, 1, 4, 0, 0);
//		se.setBuffer(data);
		return se;
	}
